...
function addDOMCards(cardsArray, sort, numIteration) {
  ...
  if (!sort) {
    ...
    cardsContainerSort.innerHTML +=
      '<div class="cards-container-sort-row"> <div class="iteration-num-container"> <p class="iteration-num">' +
      numIteration +
      "</p> </div> " +
      cardsDOM +
      " </div>";
...
function generateDOMCard(card) {
  if (["♦", "♥"].includes(card.suit)) {
    return (
      '<div class="card"> <div class="container-suit-top suit"> <p class="redColor">' +
      card.suit +
      '</p> </div> <div class="container-number"> <p>' +
      card.num +
      '</p> </div> <div class="container-suit-button suit"> <p class="redColor">' +
      card.suit +
      "</p> </div> </div>"
    );
  } else {
    return (
      '<div class="card"> <div class="container-suit-top suit"> <p class="blackColor">' +
      card.suit +
      '</p> </div> <div class="container-number"> <p>' +
      card.num +
      '</p> </div> <div class="container-suit-button suit"> <p class="blackColor">' +
      card.suit +
      "</p> </div> </div>"
    );
  }
...
const bubbleSort = cardsArray => {
  ...
  while (wall > 0) {
    ...
    while (index < wall) {
      if (
        numsCard.indexOf(newCardArray[index].num) >
        numsCard.indexOf(newCardArray[index + 1].num)
      ) {
